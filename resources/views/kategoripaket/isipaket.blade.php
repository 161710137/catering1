        ...
        <section id="news" class="blog_section bd-bottom padding">
            <div class="container">
                <div class="section_heading align-center mb-40">
                    <h2><center>{{$kategori_paket->kategori}}</center></h2>
                </div><!-- /.section_heading -->
                <div class="col-sm-4 col-xs-6 xs-padding">
                    <div class="blog_post">
                        <!-- <img src="img/blog-1.jpg" alt="blog-image"> -->
                        @foreach($kategori_paket->paket()->get() as $data)
                        <div class="blog_content">
                            <h4>{{$data->nama_paket}}</h4>
                            @php $no = 1; @endphp
                            @foreach($data->makanan as $makanan)
                            <P>{{ $no++ }}.{{$makanan->makanan}}</P>
                            @endforeach
                            <h4 class="price">Rp.{{number_format($data->harga)}}</h4>
                        </div>
                        @endforeach
                    </div>
                </div>
